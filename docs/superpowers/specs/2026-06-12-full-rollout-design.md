...
4. **Decay:** a game with no full rollout in the last 6 weeks starts losing installs/rating, worsening with staleness. Live-ops pressure is the engine of the game.
...
- Initial formula sketches (tunable constants, not contracts):
  ...
  - D1 ≈ 20% + 0.35×quality; crash% rises with missed bugs; rating delta ≈ (quality − 55)/25.
...
- **Releases screen:** per game — version, installs/rating/ARPDAU with trend, pending soft launches with metric cards and **Full Rollout / Pull Back & Fix** buttons, release history.
